...
enum PlistFileLoader {
  static func loadPropertyListSerialization<T>(with name: String, bundle: Bundle = Bundle.main) throws -> T {
  ...
  static func loadPropertyListDecodable<T: Decodable>(with name: String, bundle: Bundle = Bundle.main) throws -> T {
    guard let path = bundle.path(forResource: name, ofType: "plist"),
          let plistXML = FileManager.default.contents(atPath: path) else {
      throw FileLoaderError.fileNotFound(name: name)
    }
